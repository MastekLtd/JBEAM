...
public class AppDAO extends BaseDAO implements IAppDao {
	...
	@DatabaseAgnosticCandidate	
	private final String GET_BATCH_ERROR_SEQUENCE = "{call [$SSMA_sp_get_nextval_error_seq](?) }";//"select error_seq.NEXTVAL from dual";
...
//				pstmt.setString(12, configurable.getJobDesc());
			...
			for(int j=0;j<i.length;j++){
				logger.debug("AppDAO : " + j +" query : "+i);
			}
			return 0;
		}catch(Exception e){
			logger.debug("AppDAO : ", e);
	...
	public Boolean moreJobsToExecute(Date batchRunDate, EntityParams entityParams,
		...
		logger.debug("AppDAO : ISALL : "+isAll);
		try{					
			pstmt = connection.prepareStatement(query);
			...
			if(!isAll){
				...
				for(GroupInfo gi : values){								
					...
					for(int i=0; i<entityParams.getLookupColumns().size(); i++){
						pstmt.setTimestamp(++counter, day.getTimestamp());
						logger.debug("AppDAO : Query input : "+counter+" : "+day.getTimestamp());
						for (String str : gi.getEntityValues()) {
							pstmt.setString(++counter, str);
							logger.debug("AppDAO : Query input : "+counter+" : "+str);
						}
					}
					rs = pstmt.executeQuery();
					if(rs.next()){
			...
			logger.debug("AppDAO : MoreJob : "+returnIndicator);
	...
	public String getBatchErrorSequence(Connection connection) throws DatabaseException {
		...
		try {
			...
			*/
			CallableStatement callable = connection.prepareCall(GET_BATCH_ERROR_SEQUENCE);
