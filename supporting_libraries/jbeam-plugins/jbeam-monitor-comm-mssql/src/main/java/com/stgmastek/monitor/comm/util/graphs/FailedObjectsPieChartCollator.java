...
public class FailedObjectsPieChartCollator extends Collator {
	...
	private final String insertQuery = "insert into graph_data_log select ?, ?, ?, ?, ?, object_name 'graph_x_axis',null, count(1) from log WHERE installation_code = ? AND CONVERT(varchar(100), batch_no )= ? AND CONVERT(varchar(100),batch_rev_no) = ? AND status = ? GROUP BY OBJECT_NAME";
	...
	private void poll() throws CommException {
		...
		/*	pstmtInsert.setString(10, collatorKey.getInstallationCode());
			...
			logger.debug(pstmtInsert.toString());
