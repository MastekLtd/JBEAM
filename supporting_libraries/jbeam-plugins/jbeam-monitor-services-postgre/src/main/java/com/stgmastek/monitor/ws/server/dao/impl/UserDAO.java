...
public class UserDAO extends BaseDAO implements IUserDAO {
	...
	/** Query to get role details */
	private static final String GET_USER_INSTALLATION_ROLE_DETAILS = " SELECT user_id, installation_code, role_id FROM user_installation_role  WHERE user_id = coalesce(?, user_id) ";
//		" UNION ALL " +
...
//		" AND CONNECT_ROLE = 'Y'";
	...
	/** Query to get user details */
	  ...
	  private static final String USER_AUTHENTICATION = "SELECT user_id, user_name, telephone_no, fax_no, email_id,eff_date, exp_date, created_on, created_by, hint_question, hint_answer, password, force_password_flag, admin_role, connect_role, default_view FROM user_master WHERE user_id = ? AND password = coalesce(?, password) AND eff_date <= current_date AND exp_date >= current_date";
