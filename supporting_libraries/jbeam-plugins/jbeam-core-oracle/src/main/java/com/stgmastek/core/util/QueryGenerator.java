...
public final class QueryGenerator {
	...
	public String moreJobsExist(EntityParams entityParams, Boolean paramterized){
		...
		for(int i=0; i<columnLookups.size(); i++ ){
			...
			sb.append(" select '1' from dual where exists ( select '1' from JOB_SCHEDULE ");
			sb.append(" where 1 = 1 ");
...
//		sb.append(" be_subline subline,");
		...
		sb.append(" to_char(policy_renew_no) policy_renew_no ,");
		sb.append(isInnerJoinNeeded ? "to_char(OBE.job_seq) \"sequence\"," : " to_char(job_seq) \"sequence\",");
...
//		sb.append(" be_cash_batch_revision_no cash_batch_revision_no,");
		sb.append(" to_char(bill_no) gbi_bill_no,");
...
//		sb.append(" be_job_desc job_desc,");
		...
		sb.append(" to_char(sub_reference_id) subreference_id,");
