...
public class AppDAO extends BaseDAO implements IAppDao {
	...
	@DatabaseAgnosticCandidate	
	private final String GET_BATCH_ERROR_SEQUENCE = "select NEXTVAL('error_seq')";
	...
	public List<BatchObject> getAssignedBatchObjects(Date batchRunDate, 
		...
		try {			
			...
			for(int i=0; i<entityParams.getLookupColumns().size(); i++){
				pstmt.setInt(++counter, listenerId);
				pstmt.setTimestamp(++counter, day.getTimestamp());
				logger.debug("Adding for : "+entityParams.getLookupColumns().get(i));
				
				for (int j=0;j <groupInfo.getEntityValues().length;j++) {
					String str=groupInfo.getEntityValues()[j];
					//logger.debug("Entity values : "+str);
					String colunmName=entityParams.getLookupColumns().get(i).getValueColumns()[j];
					if("PRIORITY_CODE_1".equalsIgnoreCase(colunmName) || "PRIORITY_CODE_2".equalsIgnoreCase(colunmName) || "SUB_REFERENCE_ID".equalsIgnoreCase(colunmName) || 
					"system_transaction_seq".equalsIgnoreCase(colunmName)|| "group_system_code".equalsIgnoreCase(colunmName)|| "account_system_code".equalsIgnoreCase(colunmName) || 
					"broker_system_code".equalsIgnoreCase(colunmName)|| "policy_renew_no".equalsIgnoreCase(colunmName) || "policy_term_id".equalsIgnoreCase(colunmName)|| 
					"bill_no".equalsIgnoreCase(colunmName)|| "policy_id".equalsIgnoreCase(colunmName) || "reference_sequence".equalsIgnoreCase(colunmName) || "job_seq".equalsIgnoreCase(colunmName))
					{
					  pstmt.setInt(++counter, Integer.parseInt(str));
					}
					else
...
//				pstmt.setString(12, configurable.getJobDesc());
	...
	public Integer setBatchObjectStatus(BatchObject batchObject,
		...
		try{
			...
			pstmt.setInt(3, Integer.parseInt(batchObject.getSequence()));
	...
	public String checkBatchObjectStatus(BatchObject batchObject,
		...
		try{					
			...
			pstmt.setInt(1, Integer.parseInt(batchObject.getSequence()));
				...
				for(GroupInfo gi : values){								
					...
					for(int i=0; i<entityParams.getLookupColumns().size(); i++){
						...
						/*for (String str : gi.getEntityValues()) {
							//pstmt.setString(++counter, str);
							//logger.debug("params for moreJobsExist::"+str);
							pstmt.setInt(++counter,Integer.parseInt(str));
						}*/
						for (int j=0;j <gi.getEntityValues().length;j++) {
							String str=gi.getEntityValues()[j];
							//logger.debug("Entity values : "+str);
							String colunmName=entityParams.getLookupColumns().get(i).getValueColumns()[j];
							if("PRIORITY_CODE_1".equalsIgnoreCase(colunmName) || "PRIORITY_CODE_2".equalsIgnoreCase(colunmName) || "SUB_REFERENCE_ID".equalsIgnoreCase(colunmName) || 
							"system_transaction_seq".equalsIgnoreCase(colunmName)|| "group_system_code".equalsIgnoreCase(colunmName)|| "account_system_code".equalsIgnoreCase(colunmName) || 
	...
	public Integer updatePrePostListing(int batchNo, Date batchRunDate, 
		...
		try {			
			...
			while(rs.next()){			
				...
				pstmtUpdate.setInt(1, ++listenerNumber);
				pstmtUpdate.setInt(2, Integer.parseInt(sequence));
			...
			if(isAll){
				...
				while(rs.next()){
					...
					for(int i=0; i<pstmtUpdates.size(); i++){
						pstmtUpdates.get(i).setInt(1, Integer.valueOf(listenerNumber));
				...
				for(GroupInfo gi : values){
					...
					for(int i=0; i<pstmtUpdates.size(); i++){
						int iStmtIndex = 0;
						pstmtUpdates.get(i).setInt(++iStmtIndex, Integer.valueOf(listenerNumber));
						//logger.debug("value of listenerCount::"+Integer.valueOf(listenerCount));
						...
						for(int j=0;j<gi.getEntityValues().length;j++)
						{
							String str=gi.getEntityValues()[j];
							String colunmName=entityParams.getLookupColumns().get(i).getValueColumns()[j];
							//logger.debug("colunmName::"+colunmName);
							...
							"bill_no".equalsIgnoreCase(colunmName)|| "policy_id".equalsIgnoreCase(colunmName) || "reference_sequence".equalsIgnoreCase(colunmName)|| "job_seq".equalsIgnoreCase(colunmName))
							{
								pstmtUpdates.get(i).setInt(++iStmtIndex, Integer.parseInt(str));	
							}
							else
							{
...
//			System.err.println(query);
	...
	public Boolean markUC(Connection connection, BatchObject batchObject)
		...
		try {
			...
			pstmt.setTimestamp(1, JBeamTimeFactory.getInstance().getCurrentTimestamp(connection));
			pstmt.setInt(2, Integer.parseInt(batchObject.getSequence()));
