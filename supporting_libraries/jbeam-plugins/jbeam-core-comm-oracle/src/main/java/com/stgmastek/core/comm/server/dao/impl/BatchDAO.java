...
public class BatchDAO extends BaseDAO implements IBatchDAO {
	...
	private static final String PRE_GET_REQUEST_ID = "select process_req_seq.nextval from dual";
	...
	private static final String GET_INSTRUCTION_LOG = 
		"SELECT seq_no, batch_action, batch_action_time, message_param " +
...
//	" and prs.sch_id = pr.req_id";
	...
	public Integer addInstructionLog(CReqInstructionLog reqInstructionLog, Connection connection) 
		...
		try{
			...
			//Let it be false as the decision to check if its locked or not will be taken later
			//It do not solely depend on batch_lock table
			//batchLocked = isBatchLocked(connection);
			
			reqInstructionLog.setBatchAction((batchLocked?CommConstants.BATCH_LOCKED:CommConstants.BATCH_REQUESTED));
