...
{	
import com.majescomastek.common.containers.dashboard.DragHighlight;
...
import com.majescomastek.common.events.PodStyleChangeEvent;
...
[Event(name="update", type="com.majescomastek.common.events.LayoutChangeEvent")]
	...
	public class PodLayoutManager extends EventDispatcher
	{
		...
		public var closedItems:Array = new Array();			// Stores the closed pods.
		...
		public function addClosedItemAt(pod:Pod, index:Number):void
		{	
			if (index == -1)
				index = closedItems.length;
			
			pod.close();
			
			closedItems[index] = pod;
			initItem(pod);
		}
		...
		private function initItem(pod:Pod):void
		{
			...
			pod.addEventListener(PodStyleChangeEvent.HEADER_HEIGHT,onChangeHeaderHeight);
			Application.application.stage.addEventListener(PodStateChangeEvent.ADDALL, onAddAll);
		...
		//Change the header height to 0 of all other Pods except the Pod who trigerred the event.
		private function onChangeHeaderHeight(e:PodStyleChangeEvent):void{
			var currentPod:Pod = Pod(e.target);
			for(var i:Number=0;i<items.length;i++){
				var pod:Pod = Pod(items[i]);
				if(currentPod.id != pod.id){
					pod.setStyle("headerHeight","0");
					pod.showControls = false;
				}
			}
		}
		...
		private function onMaximizePod(e:PodStateChangeEvent):void
		{
			...
			maximizedPod = pod;
			//Added on 28-May-10 Mandar Start
			//This will exclusively set the index of maximized pod to the last index.
			container.setChildIndex(pod,container.getChildren().length-1);
			//Added on 28-May-10 Mandar End
			dispatchEvent(new LayoutChangeEvent(LayoutChangeEvent.UPDATE));
		}
		
		// All Pods has been added.
		private function onAddAll(e:PodStateChangeEvent):void
		{
			var pod:Pod;
			try
			{
				...
				else if (pod.windowState == Pod.WINDOW_STATE_CLOSED) // Current state is closed so add it back to the display.
				{
					len = closedItems.length;
					for (i = 0; i < len; i++)
					{
						// Remove the minimized window from the minimized items.
						if (closedItems[i] == pod)
						{
							closedItems.splice(i, 1);
							break;
						}
					}
		...
		private function onMouseMove(e:MouseEvent):void
		{
			...
			for (var i:Number = 0; i < len; i++)
			{
				...
				//if (currentDragPod.hitTestObject(dragHighlightItem))	{
					//ExternalInterface.call("function(){ window.status=\"Inside draghightlight condition\";};");
		...
		// Lays out the pods, minimized pods, closes pods and drag highlight items.
		public function updateLayout(tween:Boolean=true):void
		{
			...
			//--------------------------------------------------------------------------------------------------
			// Layout the closed items.
			len = closedItems.length;
			if (len > 0)
			{
				// Check to see if all of the minimized items will be too wide.
				totalMinimizedItemWidth = len * TASKBAR_ITEM_WIDTH + (len -1) * TASKBAR_HORIZONTAL_GAP;
				//var minimizedItemWidth:Number;
				if (totalMinimizedItemWidth > availablePodWidth) // Items are too wide so resize.
					minimizedItemWidth = Math.round((availablePodWidth - (len - 1) * TASKBAR_HORIZONTAL_GAP) / len);
				else
				...
				for (i = 0; i < len; i++)
				{
					pod = Pod(closedItems[i]);
					//Alert.show("Pod "+ i + "-> Height = " + pod.height);
					pod.height = 0;
					targetX = i * (TASKBAR_HORIZONTAL_GAP + minimizedItemWidth);
					if (tween)
					{
						addResizeAndMoveToParallel(pod, parallel, minimizedItemWidth, 0, targetX, minimizedPodY);
					}
					else
					{
						pod.width = 0;
		...
		//Displays the pods 
		public function showPods(arrPods:Array):void 
		{
			var index:Number;
			...
			if(arrPods.length > 0)
			{
				for(var i:Number = 0; i<arrPods.length; i++)
				{
					pod = arrPods[i];
					if(pod.windowState == 2)
					{
						index = StateManager.getClosedPodIndex(this.id,pod.id);
						this.addClosedItemAt(pod, index);
					}
					else
					{
						index = StateManager.getPodIndex(this.id,pod.id);
			
						// If the index hasn't been saved move the pod to the last position.
						if (index == -1) 
						{
							index = arrPods.length + unsavedPodCount;
							unsavedPodCount += 1;
						}						
						this.addItemAt(pod, index, StateManager.isPodMaximized(this.id,pod.id));
					}
			...
			{
				AlertBuilder.getInstance().show("No Batch details available");
			}
		}
