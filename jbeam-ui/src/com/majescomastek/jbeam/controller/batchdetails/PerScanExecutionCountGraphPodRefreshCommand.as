...
package com.majescomastek.jbeam.controller.batchdetails
{
	...
	/**
	 * The command class used to refresh the per scan Execution count graph pod data.
	 * 
	 ...
	 * of the screen. 
	 */
	public class PerScanExecutionCountGraphPodRefreshCommand extends BaseSimpleCommand
	{
		override public function execute(notification:INotification):void
		{
			var data:Object = notification.getBody();
			var mediatorName:String = String(data['mediatorName']);
			var batchDetails:BatchDetailsData = BatchDetailsData(data['batchDetails']);
			
			var proxy:BatchDetailsProxy =
				BatchDetailsProxy(facade.retrieveProxy(BatchDetailsProxy.NAME));

			proxy.getPerScanExecutionCountGraphData(batchDetails, mediatorName);
		}
	}
}