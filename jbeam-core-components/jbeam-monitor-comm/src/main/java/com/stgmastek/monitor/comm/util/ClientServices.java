...
public final class ClientServices {
	...
	HashMap<String, HashMap<String, MonitorCommServices>> monitorServiceMap = null;
	...
	ClientServices() {
		...
		monitorServiceMap = new HashMap<String, HashMap<String, MonitorCommServices>>();
	}
	 ...
	 * @return the cached method ready object
	 */
	MonitorCommServices getMonitorClientClass(String installationCode) {
...
//		
		//Do a lazy caching for core comm of those installation that were down 
		// when the first caching round was performed
//		MonitorCommServices obj = map.get("SERVICES");
//		if(obj == null){
//			//Get the client stub and cache it
//			for(MConfig config : serviceConfigs){
//				if(config.getCode2().equals(installationCode)){
...
//		}
	...
	private void setMonitorEntries(List<MConfig> entries){
		for(MConfig entry : entries){
			...
			if (monitorServiceMap.containsKey(code2)) {
				HashMap<String, MonitorCommServices> map = monitorServiceMap.get(code2);
				String code3 = entry.getCode3();
				if(!map.containsKey(code3)){
					MonitorCommServices obj = stackMonitorClientStub(entry.getValue());
					if(obj != null)
						map.put(code3, obj);
				}
			} else {
	 ...
	 * @return the client stub
	 */
	private MonitorCommServices stackMonitorClientStub(String value){
		...
		try {
			...
			MonitorCommServices client = (MonitorCommServices) proxyFactory
					.create();
