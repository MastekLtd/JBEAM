...
import com.stgmastek.monitor.comm.main.StartMonitorCommunication;
...
import com.stgmastek.monitor.comm.util.IOUtils;

/**
 * The manager (or the main) class to manage all the servers in the monitor communication system. 
...
public final class WSServerManager {
	...
	class WSFactoryImpl implements WSServerFactory{
		...
		public boolean destroyService(WSServerInfo info){
			...
			for (BasePoller poller : StartMonitorCommunication.getPollers()) {
				poller.stopPoller();
			}
			WSServer server = null;
			...
			if((server = serviceBook.getServer(info)) != null){
				if(server.isRunning()) {
					try {
						CoreCommServices coreCommServices = ClientBook.getBook().getClientClass("INSTALLATION_WS");
						org.apache.cxf.endpoint.Client coreClient = ClientProxy.getClient(coreCommServices);
						coreClient.destroy();
						if (logger.isEnabledFor(LogLevel.NOTICE)) {
							logger.log(LogLevel.NOTICE, "Destroyed core client");
						}
					}catch (Exception e) {
						if (logger.isEnabledFor(LogLevel.NOTICE)) {
							logger.log(LogLevel.NOTICE, "Could not destroy core client", e);						
						}
					}					
					...
					if (logger.isEnabledFor(LogLevel.NOTICE)) {
						logger.log(LogLevel.NOTICE, "Destroyed monitor client");
					}
...
//						
					server.stop();
					if (logger.isEnabledFor(LogLevel.NOTICE)) {
						logger.log(LogLevel.NOTICE, "Stopped the " + info.getAddress().getDomain());
					}
				}
...
* Revision Log
* -------------------------------
* $Log:: /Product_Base/Projects/Batch/Code/Java/MonitorComm/src/com/stgmastek/monitor/comm/server/util/WSServerManager.java                                                          $
 * 
 * 5     6/23/10 4:42p Mandar.vaidya
 * prefixed  resources folder to the sslcert
 * 
 * 4     6/23/10 3:50p Kedarr
 * Changed the signature of the method to void of configure ssl.
 * 
